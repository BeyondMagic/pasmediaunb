...
function organizeNotes(notas, pessoas) {
  ...
  let output = `<span class="name">Pessoas que fizeram a prova:</span> ${pessoas}`;
  output = `${output}\n<span class="name">Redação:</span> ${notas[0]} de 10.000`;
  output = `${output}\n<span class="name">Tipo D:</span> ${notas[1]} de 3.000`;
  // se for definitivo
  if (notas.length === 5) {
